...
function list_Item() {
	listItem = [{name:"#name",
			...
			{name:"#other_title",
					condition:$("#other_title").val() === "" && $("#title").children("option:selected").attr("value") === "other",
						caption:"Job Role",
							errormessage:" (please enter your title)"},
							
			{name:"#color",
					condition:$("#color").children("option:selected").attr("value") === "select",
						caption:"",
							errormessage:"Don't forget to pick a T-shirt"},
			...
			{name:"#payment",
					condition:$("#payment").children("option:selected").attr("value") === "select",
						caption:"I'm going to pay with:",
...
function activitiess() {
	for (var i = 0; i < activitiesList.length; i+=1) {
		var activitie = $(".activities").children().children().eq(activitiesList[i].conflict);
			if ($(".activities").children().children()[i].checked === true) {
				$(activitie).attr("disabled", true);
				$(activitie).parent().addClass("disabled");
			} else {
				$(activitie).attr("disabled", false);
				$(activitie).parent().removeClass("disabled");						
			}
	}
}

//this function is calculating the total ammout of the selected activities
function price() {
	totalprice = 0;
	for (var i = 0; i < activitiesList.length; i+=1) {
		if ($(".activities").children().children()[i].checked === true) {
			totalprice += activitiesList[i].price;
		}
	}
	if (totalprice === 0) {
		$(".price").text("");
	} else {
		$(".price").text("Total cost : $" + totalprice);
	}
}

//this function shows or hide the "other_title" input , if "other" is selected
function othertitle() {
	if ($("#title").val() === "other") {
		$("#other_title").removeClass("is-hidden");
	} else {
		$("#other_title").addClass("is-hidden");
	}
}
	...
	if(!first_time) {
		for (var i = 0; i< listItem.length; i+=1) {
			if(listItem[i].condition) {
				$(listItem[i].name + "e").text(listItem[i].caption + listItem[i].errormessage);
				$(listItem[i].name + "e").addClass("error");
			} else {
				$(listItem[i].name + "e").text(listItem[i].caption);
				$(listItem[i].name + "e").removeClass("error");
